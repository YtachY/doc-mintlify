...
<Steps>
  ...
  <Step title="Re-implement the algorithm in Python">
    ...
    ```python
    # After reversing the check function in Ghidra:
    # flag[i] = input[i] XOR 0x42 + i

    ciphertext = [0x66, 0x6c, 0x61, 0x67, 0x7b, ...]  # extracted from binary
    ...
    Extract the encoded byte array from Ghidra's listing view or from the binary directly using Python's `open(..., 'rb')`. Run the script and check whether the output matches the expected flag format. If it's close but garbled, double-check the order of operations — XOR and arithmetic precedence in the decompiler pseudocode are common sources of off-by-one errors.
  </Step>
